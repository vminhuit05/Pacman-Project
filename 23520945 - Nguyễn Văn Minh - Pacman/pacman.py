...
ghost1_img = pygame.transform.scale(pygame.image.load(f'assets/ghost_images/gojo.png'), (45, 45))

ghost2_img = pygame.transform.scale(pygame.image.load(f'assets/ghost_images/luffy.png'), (45, 45))

ghost3_img = pygame.transform.scale(pygame.image.load(f'assets/ghost_images/nak.png'), (45, 45))

ghost4_img = pygame.transform.scale(pygame.image.load(f'assets/ghost_images/shiba.png'), (45, 45))
...
def draw_misc():
    ...
    if game_over:
        ...
        gameover_text = font.render('End game ! Space bar to restart!', True, 'red')
        screen.blit(gameover_text, (100, 300))
    if game_won:
        pygame.draw.rect(screen, 'white', [50, 200, 800, 300],0, 10)
        pygame.draw.rect(screen, 'dark gray', [70, 220, 760, 260], 0, 10)
        gameover_text = font.render('Congratuation ! Space bar to restart!', True, 'green')
        screen.blit(gameover_text, (100, 300))
...
def get_targets(ghost1_x, ghost1_y, ghost2_x, ghost2_y, ghost3_x, ghost3_y, ghost4_x, ghost4_y):
    ...
    if powerup:
        ...
        if not ghost3.dead:
            ghost3_target = (player_x, runaway_y)
        elif not ghost3.dead and eaten_ghost[2]:
        ...
        if not ghost4.dead and not eaten_ghost[3]:
            ghost4_target = (450, 450)
        elif not ghost4.dead and eaten_ghost[3]:
        ...
        if not ghost4.dead:
            ...
            ghost4_target = return_target
    return [ghost1_target, ghost2_target, ghost3_target, ghost4_target]
...
while run:
    ...
    ghost2 = Ghost(ghost2_x, ghost2_y, targets[1], ghost_speeds[1], ghost2_img, ghost2_direction, ghost2_dead,
                 ghost2_box, 1)
    ghost3 = Ghost(ghost3_x, ghost3_y, targets[2], ghost_speeds[2], ghost3_img, ghost3_direction, ghost3_dead,
                  ghost3_box, 2)
    ghost4 = Ghost(ghost4_x, ghost4_y, targets[3], ghost_speeds[3], ghost4_img, ghost4_direction, ghost4_dead,
                  ghost4_box, 3)
    draw_misc()
    targets = get_targets(ghost1_x, ghost1_y, ghost2_x, ghost2_y, ghost3_x, ghost3_y, ghost4_x, ghost4_y)
    ...
    if not powerup:
        if (player_circle.colliderect(ghost1.rect) and not ghost1.dead) or \
                ...
                (player_circle.colliderect(ghost3.rect) and not ghost3.dead) or \
                (player_circle.colliderect(ghost4.rect) and not ghost4.dead):
            if lives > 0:
                lives -= 1
                startup_counter = 0
                powerup = False
    ...
    if powerup and player_circle.colliderect(ghost2.rect) and eaten_ghost[1] and not ghost2.dead:
        if lives > 0:
            ...
            moving = False
            startup_counter = 0
    if powerup and player_circle.colliderect(ghost3.rect) and eaten_ghost[2] and not ghost3.dead:
        if lives > 0:
            powerup = False
            power_counter = 0
            lives -= 1
            ...
            moving = False
            startup_counter = 0
    if powerup and player_circle.colliderect(ghost4.rect) and eaten_ghost[3] and not ghost4.dead:
        if lives > 0:
            powerup = False
            power_counter = 0
            lives -= 1
    ...
    if powerup and player_circle.colliderect(ghost2.rect) and not ghost2.dead and not eaten_ghost[1]:
        ...
        eaten_ghost[1] = True
        score += (2 ** eaten_ghost.count(True)) * 100
    if powerup and player_circle.colliderect(ghost3.rect) and not ghost3.dead and not eaten_ghost[2]:
        ghost3_dead = True
        eaten_ghost[2] = True
        score += (2 ** eaten_ghost.count(True)) * 100
    if powerup and player_circle.colliderect(ghost4.rect) and not ghost4.dead and not eaten_ghost[3]:
        ghost4_dead = True
        eaten_ghost[3] = True
        score += (2 ** eaten_ghost.count(True)) * 100
